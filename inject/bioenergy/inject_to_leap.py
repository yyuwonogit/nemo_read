"""Bioenergy-domain LEAP injector — thin subclass of CanonicalInjector.
...
nemo_read.inject_base. This file owns only bioenergy-specific concerns:
...
Usage:
    python inject/bioenergy/inject_to_leap.py --dry-run
...
def _validate_leap_native(csv_path: Path) -> list[str]:
    ...
    if name == "canonical_leap_inputs.csv" and native.exists():
        return [
            ...
            f"inject/bioenergy/run_workflow.py step 4). Pass "
...
if __name__ == "__main__":
    raise SystemExit(BioenergyInjector().run())
