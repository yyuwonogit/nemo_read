...
| Eff leaf | **Exogenous Devices** (Device) | All | `device_thousand` (× 1000 → Device), **every year 2005–2060**, from `fridge_exo_device.csv` | **Updated 2026-06-30.** Target slot is the leaf **`Exogenous Devices`** variable (FRIDGE_ANATOMY.md §1.3b / §4). Carries the **existing fleet only, with Weibull retirement and NO additions** — LEAP's optimiser supplies all device additions. The 2025 fleet (= LEAP RAS Demand Devices export, ASEAN 146,579 k) is frozen and retired forward (Weibull k=3, mean 12 yr) declining to ~0 by ~2050. Per-year series in `fridge_exo_device.csv`; underlying 2005–2025 stock + backed-out ownership in `fridge_device_numbers.csv`. `device_thousand` is in thousands → × 1000 for the `Device`-unit slot. |
