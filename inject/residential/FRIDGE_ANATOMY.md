...
> AC companion (mirrors this structure): [AC_ANATOMY.md](AC_ANATOMY.md).
...
⚠ **Two leaf variable sets exist in TWO different areas** (confirmed by COM
probe + the 2026-06-29 dry-run):
- **`aeo9_v0.64`** (base area) = **device-stock leaf** (§1.3b) — has
  `Efficiency`, `Exogenous Devices`, `Unit Capacity`, Capital/OM/Lifetime;
  **no `Demand Cost`**. **This is the current inject target.**
- **`aeo9_v0.64_w_result`** (results copy) = **simple leaf** (§1.3a) — has
  `Activity Level`, `Efficiency`, `Demand Cost`; **no device-stock vars**.
...
Authorable inputs on THIS (`_w_result`) leaf: **`Activity Level`** (eff-tier
share), **`Efficiency`**, **`Demand Cost`**. No device-stock vars here
(`Unit Capacity` / `Capital Cost` / `Fixed OM` / `Variable OM` / `Lifetime` /
`Exogenous Devices` / `Maximum Devices`) — those are on the §1.3b leaf.

#### 1.3b Device-stock leaf — `aeo9_v0.64` (base area, CURRENT inject target)  [seen + dry-run-confirmed 2026-06-29]
...
> Confirmed 2026-06-29: a dry-run against `aeo9_v0.64` returned `Efficiency`
> present, `Demand Cost` absent — i.e. `aeo9_v0.64` IS this device-stock leaf.
> **Phase-2 inject target here (DONE 2026-06-29, RAS, readback EXACT):
> **FULL inject DONE 2026-06-30** (RAS, readback EXACT): Key drivers +
> `Efficiency` (all scenarios) + the complete RAS device-stock block —
> `Unit Capacity`, `Capital Cost` (`price_usd`, full — LEAP annualizes by
> Lifetime), `Variable OM Cost` (`om_electricity_usd`), `Fixed OM Cost`=0,
> `Lifetime`=12, `Exogenous Devices` (2005→2060). 1290 writes, 30/30 EXACT.
> See ../LAST_SUCCESSFUL_INJECT.md for the repeatable recipe.
...
| `efficiency_pct` | leaf `Efficiency` | % | **INJECTED 2026-06-30** (all scenarios) |
| `unit_capacity_kw` (per size) | leaf `Unit Capacity` | kW | **INJECTED 2026-06-30** (RAS-only) |
| `fridge_exo_device.csv` 2005-2060 (×1000) | leaf `Exogenous Devices` | Device | **INJECTED 2026-06-30** — 2005→2060 series, RAS-only |
| `price_usd` | leaf `Capital Cost` | USD | **INJECTED 2026-06-30** (RAS-only; FULL capital — LEAP annualizes by Lifetime) |
| `om_electricity_usd` | leaf `Variable OM Cost` | USD | **INJECTED 2026-06-30** (RAS-only) |
| `0` | leaf `Fixed OM Cost` | USD | **INJECTED 2026-06-30** (RAS-only) |
| `12` | leaf `Lifetime` | Years | **INJECTED 2026-06-30** (RAS-only) |
...
**Resolved since:** device-stock leaf area/version — `aeo9_v0.64` IS the
device-stock leaf (§1.3b), confirmed by the 2026-06-29 dry-run + the 2026-06-30
AC probe. **AC (Air Conditioning) now verified** — see [AC_ANATOMY.md](AC_ANATOMY.md)
(mirrors this structure: `Air Conditioning_` Demand tree + `Key\Residential\Air
Conditioning` 16-node store + device-stock leaf).

**Still not verified:** live-leaf input *expressions/values* (only names probed —
