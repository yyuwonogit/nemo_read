...
def main() -> int:
    ...
    if n_fail:
        return 1
    if n_norm:
        # CLAUDE.md §A.15: any NORMALISED match means the inject committed
        # the wrong Interp separator (semicolon or period list-sep instead
        # of comma). This is a HARD FAIL — do not proceed to
        # calculatescenario. Fix the inject path and re-push.
        print(f"[probe] FAIL: {n_norm} probe(s) only matched after "
              f"separator normalisation — inject committed wrong "
              f"Interp() form (§A.15). Do NOT run calculatescenario.")
        return 1
    return 0
